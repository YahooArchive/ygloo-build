...
$(hide) $(PRIVATE_CXX) \
	...
	$(PRIVATE_DEPEND_CFLAGS) -MF $(patsubst %.o,%.d,$@) -o $@ $<
$(transform-d-to-p)
endef
...
# Explicitly delete the archive first so that libtool doesn't
# try to add to an existing archive.
define transform-a-to-static-lib
@mkdir -p $(dir $@)
@rm -f $@
...
# Explicitly delete the archive first so that libtool doesn't
# try to add to an existing archive.
define transform-a-to-fat-lib
@mkdir -p $(dir $@)
@rm -f $@
$(extract-and-include-target-whole-static-libs)
@echo "target StaticFatLib: $(PRIVATE_MODULE) ($@)"
$(call split-long-arguments,$(TARGET_LIPO) $(TARGET_GLOBAL_LIPOFLAGS) $(PRIVATE_LIPOFLAGS) -create,$(filter %.a, $^) -output $@)
endef
